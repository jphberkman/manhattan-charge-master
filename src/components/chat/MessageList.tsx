                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation":
                            ...
                            const toolLabels: Record<string, { doing: string; done: string }> = {
                              str_replace_editor: { doing: "Writing code...", done: "Code written" },
                              file_manager: { doing: "Managing files...", done: "Files updated" },
                            };
                            const label = toolLabels[tool.toolName] ?? { doing: "Working...", done: "Done" };
                              ...
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {isDone ? (
                                  ...
                                  <>
                                    ...
                                    <span className="text-neutral-600">{label.doing}</span>
                                  </>
                                )}
                              </div>
                            );
